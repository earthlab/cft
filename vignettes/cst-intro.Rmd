...
There are a three steps required for the `cst` package to work:
...
required. In `cst` these are accessed automatically through the `reticulate` R
package, and you will not need to know any Python, but you will need to have a
working installation. If you don't have Python on your machine we recommend using
Conda because it is the simplest way to ensure a functioning installation and it
manages dependencies well. First make sure you have conda installed 
...
> Alternatively, for users with existing Python installations who prefer a quicker
method, you may use the `virtualenv` package to store your Python dependencies:

```r
cst::install_py_deps(method = "virtualenv")
reticulate::use_virtualenv("cst")
```

> Both methods will create isolated virtual python environments stored in your home
directory in a folder called "cst". To access the paths associated with these
environments use :
...
the NPS. Use the `cstdata()` function to specify a range of years, a set of models,
...
#> Error in cstdata(park = "Wind Cave National Park", years = c(1980, 2040), : could not find function "cstdata"
```
...
```r
glimpse(file_refs)
#> Error in glimpse(file_refs): could not find function "glimpse"
```
...
#> Error in cst_df(file_refs, ncores = parallel::detectCores()): could not find function "cst_df"
```
...
columns. Note that, in the output of `cst_df()` temperature values have been
converted from Kelvin to Celsius and that average values represent all 2km by
...
#> Error in df %>% filter(date > Sys.Date(), date < Sys.Date() + 365 * 5, : could not find function "%>%"
```
...
#> Error in df %>% mutate(tasmid = (tasmax + tasmin)/2): could not find function "%>%"
```
...
#> Error in df %>% mutate(wind_speed = sqrt(vas^2 + uas^2)): could not find function "%>%"
```
...
#> Error in df %>% mutate(year = year(date)): could not find function "%>%"
```
...
growing_seasons <- df %>%
  ...
  ungroup
#> Error in df %>% group_by(rcp, model, year, ensemble) %>% summarize(season_length = sum(tasmid > : could not find function "%>%"
```
...
#> Error in growing_seasons %>% ggplot(aes(year, season_length, color = rcp, : could not find function "%>%"
```


## Comparing climate in two time periods

Use the tibble object that is returned from `cst_df()` as an input to
...
#> Error in compare_periods(df, var1 = "tasmin", var2 = "tasmax", agg_fun = "mean", : could not find function "compare_periods"
```
...
```r
glimpse(comps)
#> Error in glimpse(comps): could not find function "glimpse"
```
...
comps %>%
  ...
  scale_color_manual(values = c("dodgerblue", "red"))
#> Error in comps %>% dplyr::select(parameter, rcp, model, year1, year2, : could not find function "%>%"
```
...
The Climate Scenarios Toolbox was developed to help users access and use 
smaller subsets. 
