...
You can install miniconda with the [reticulate](https://rstudio.github.io/reticulate/) R package: 
...
After installing miniconda, python dependencies can be installed as follows: 
...
```{r, include = FALSE}
# this 'secret' code chunk sets a local directory so that the data aren't 
# downloaded every time we build the README.md file, but it would be impolite
# to write to the user's home directory for this example without permission.
d <- cftdata(park = "Acadia National Park", 
             parameters = "pr", local_dir = "~",
