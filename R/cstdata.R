...
cstdata <- function(parkname="Yellowstone National Park"){
  ...
  pb <- progress_bar$new(total = length(file_names))
  for (i in seq(length(purls))){
    pb$tick(0)
    ...
    if (!file.exists(dst)) {
      ...
      url <- c(hurls[[i]], murls[[i]])

      # Save a local file
      ds <- xr$open_mfdataset(url, concat_dim="time")
      ...
      attrs2 <- list(
                  ...
                  "geospatial_lat_resolution" = grid$resolution,  # <------------ It wasn't as precise in theirs
                  "geospatial_lon_resolution" = grid$resolution,  # <------------ It wasn't as precise in theirs
